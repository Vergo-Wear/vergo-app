...
export interface OrderItem { id: string; customerName: string; customerEmail: string; customerPhone: string; customerAddress: string; timestamp: string; paymentMethod: "COD" | "BANK"; status: "Ready to Pick" | "Claimed" | "Preparing" | "Ready for Pickup" | "Sent"; dbStatus: string; valuation: number; initials: string; claimedBy: string | null; stockAvailable: boolean; stockShortages: string[]; itemsList: { description: string; size: string; color: string; qty: number; unitPrice: number; sku: string }[] }
...
interface EmployeeContextType {
  ...
  claimTask: (id: string) => Promise<boolean>; claimOrder: (id: string) => Promise<boolean>; startPrep: (sku: string) => void; updatePrepStatus: (sku: string, status: "PREPARING" | "NOT STARTED" | "READY") => void; finalizeDelivery: (id: string) => void; notifyCustomer: (id: string) => void; toggleAvailability: () => void; notifyAvailabilityToAdmin: () => void; setDimensions: (dims: { length: string; width: string; height: string }) => void; setWeight: (w: string) => void; setPickupSlot: (slot: string) => void; triggerWaybillGeneration: () => void; triggerPickupRequest: () => void; selectActiveDeliveryPrep: (id: string) => void; addNotification: (title: string, message: string, type: NotificationItem["type"]) => void; clearNotifications: () => void; requestStockFromAdmin: (sku: string, quantity: number) => void; logoutEmployee: () => void;
}

interface RawOrder { orderId: string; employeeId: string | null; orderDate: string | null; orderStatus: string | null; confirmationStatus: string; totalAmount: string | number; paymentMethod: string; shippingAddress: string; stockAvailable?: boolean; stockShortages?: string[]; customerDetails: { firstName: string; lastName: string; email: string; phone: string } | null; orderItems: Array<{ quantity: number; unitPrice: string | number; variant: { sku: string; size: string; color: string; product: { name: string } | null } | null }> }
...
export function EmployeeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setOrders((rawOrders as RawOrder[]).map(mapOrder).filter((order): order is OrderItem => order !== null));
  ...
  const updateOrderStatus = async (id: string, status: OrderItem["status"]) => {
    const auth = token();
    if (!auth) {
      addNotification("Login required", "Your employee session has expired. Please sign in again before claiming an order.", "error");
      return false;
    }
    try {
      const endpoint = status === "Claimed" ? `${API_URL}/orders/manage/${id}/claim` : `${API_URL}/orders/manage/${id}/status`;
      const response = await fetch(endpoint, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${auth}` }, ...(status === "Claimed" ? {} : { body: JSON.stringify({ status }) }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.message || "Unable to update order.");
      const persistedStatus = body.orderStatus === "Ready to Process" ? "Ready to Pick" : body.orderStatus || status;
      setOrders((current) => current.map((order) => order.id === id ? { ...order, status: persistedStatus as OrderItem["status"], dbStatus: body.orderStatus || status, claimedBy: persistedStatus === "Claimed" ? "Current employee" : order.claimedBy } : order));
      return true;
    } catch (error) {
      addNotification("Order update failed", error instanceof Error ? error.message : "Unable to update order.", "error");
      return false;
    }
  };
  const claimOrder = async (id: string) => {
    const claimed = await updateOrderStatus(id, "Claimed");
    if (claimed) addNotification("Order claimed", "The parcel is now available in Product Prep.", "success");
    return claimed;
  };
  ...
  const productPrepQueue: ProductPrepItem[] = orders.filter((order) => ["Claimed", "Preparing"].includes(order.status)).flatMap((order) => order.itemsList.map((item) => ({ id: `${order.id}-${item.sku}`, name: item.description, sku: item.sku, size: item.size, color: item.color, imageColor: "#111111", quantity: item.qty, orders: `Order #${order.id}`, assignedTo: order.claimedBy || "Current employee", assignedInitials: "", status: order.status === "Preparing" ? "PREPARING" : "NOT STARTED" }))) as ProductPrepItem[];
