...
CREATE POLICY "cart_item_delete_policy" ON public."cart_item" FOR DELETE TO public USING ((EXISTS ( SELECT 1
   FROM cart c
  WHERE ((c.cart_id = c.cart_id) AND ((c.customer_id = (select private.get_customer_id())) OR (c.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee()))))));
...
CREATE POLICY "cart_item_update_policy" ON public."cart_item" FOR UPDATE TO public USING ((EXISTS ( SELECT 1
   FROM cart c
  WHERE ((c.cart_id = c.cart_id) AND ((c.customer_id = (select private.get_customer_id())) OR (c.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee())))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM cart c
  WHERE ((c.cart_id = c.cart_id) AND ((c.customer_id = (select private.get_customer_id())) OR (c.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee()))))));
...
-- Removed contact_message_insert_policy to prevent unauthenticated direct REST inserts.
...
CREATE POLICY "review_image_delete_policy" ON public."review_image" FOR DELETE TO authenticated USING ((EXISTS ( SELECT 1
   FROM review r
  WHERE ((r.review_id = r.review_id) AND ((r.customer_id = (select private.get_customer_id())) OR (select private.is_admin()) OR (select private.is_employee()))))));
...
CREATE POLICY "review_image_update_policy" ON public."review_image" FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1
   FROM review r
  WHERE ((r.review_id = r.review_id) AND ((r.customer_id = (select private.get_customer_id())) OR (select private.is_admin()) OR (select private.is_employee())))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM review r
  WHERE ((r.review_id = r.review_id) AND ((r.customer_id = (select private.get_customer_id())) OR (select private.is_admin()) OR (select private.is_employee()))))));
