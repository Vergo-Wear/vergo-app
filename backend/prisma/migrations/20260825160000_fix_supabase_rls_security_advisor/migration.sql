...
CREATE POLICY "category_insert_policy" ON public."category" FOR INSERT TO authenticated WITH CHECK (((select private.is_admin()) OR (select private.is_employee())));
CREATE POLICY "category_update_policy" ON public."category" FOR UPDATE TO authenticated USING (((select private.is_admin()) OR (select private.is_employee()))) WITH CHECK (((select private.is_admin()) OR (select private.is_employee())));
CREATE POLICY "category_delete_policy" ON public."category" FOR DELETE TO authenticated USING (((select private.is_admin()) OR (select private.is_employee())));
...
CREATE POLICY "order_customer_details_insert_policy" ON public."order_customer_details" FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM orders o
  WHERE ((o.order_id = order_customer_details.order_id) AND ((o.customer_id = (select private.get_customer_id())) OR (o.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee()))))));
...
CREATE POLICY "order_item_insert_policy" ON public."order_item" FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM orders o
  WHERE ((o.order_id = order_item.order_id) AND ((o.customer_id = (select private.get_customer_id())) OR (o.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee()))))));
...
CREATE POLICY "order_shipping_details_insert_policy" ON public."order_shipping_details" FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM orders o
  WHERE ((o.order_id = order_shipping_details.order_id) AND ((o.customer_id = (select private.get_customer_id())) OR (o.customer_id IS NULL) OR (select private.is_admin()) OR (select private.is_employee()))))));
